...
**New features:**
- Added `GET /machines` with optional `?from=<worker>` for caller-aware access hints.
- Added `machines.json` loading with the v1 SDD schema at `~/.config/claudecode-telegram/machines.json`.
- Added optional machine metadata: `display_name`, `tailscale_ip`, and `role`.

**Architecture notes:**
- `/machines` uses the SDD's static catalog shape. The future `Machine`/`MachineAdapter` work can reuse the same source file.
- Worker placement comes from `workers.json` and tmux/session discovery. The endpoint does not mutate machine or worker state.
- Unknown legacy `host` values appear as `configured=false` machines. Operators can see the gaps instead of losing workers from the view.
...
**New features:**
- Added `POST /send` for manager-side tools to deliver `{worker, message}` prompts through `send_to_worker()`.
- Added `mcp_server.py`, a minimal stdio MCP server for Claude Code with `list_workers`, `send_message`, and `get_response` tools.

**Architecture notes:**
- Worker-to-worker messaging stays decentralized through `/workers` discovery and direct tmux/pipe commands.
- `/send` is a manager control-plane endpoint for MCP integrations. It does not replace direct worker peer messaging.

**Tests:**
- Added `test_send_endpoint_delivers_to_worker` with real tmux delivery through `POST /send`.

### v0.30.1 - Caller-aware `/workers?from=<name>` for cross-machine sends

**Problem:** `/workers` returned a bridge-POV `send_example` (for example, `tmux paste-buffer ...`). That fails when the caller lives on a different machine than the bridge. Example: kai (Mac Mini) queried `/workers` for mon (VPS/bridge), got a bare tmux command, and ran it locally. The command hit "session not found" because mon's tmux lives on VPS, not Mac Mini.

**Why not centralize sends via `POST /send`?** That would break the "decentralized worker comms" principle. The bridge is a discovery service, not a router. Workers run their own tmux/SSH commands directly.

**Fix (minimal — ~30 lines, zero new endpoints):**
- `GET /workers?from=<caller_name>` — the bridge resolves the caller's machine and renders each peer's `send_example` from the caller's point of view.
- New `_wrap_for_caller(cmd, peer_host, caller_host)` helper handles four cases:
  - Same machine (both bridge-local): bare command, no ssh
...
- Each worker dict now includes a `machine` field (empty string for bridge-local).
- Welcome message tells workers to call `curl -s "$BRIDGE_URL/workers?from=<your_name>"` instead of bare `/workers`.
...
**Problem:** VPS-cached `claude_session_id` for teleported workers kept going stale. `/rewind`, `/restart`, and `/memory` handed out UUIDs whose JSONL file no longer existed on the worker's current host. The cache was trusted without validation. The SSH fallback wrote stale values back into the VPS cache. This made the staleness permanent.

**Root cause:** `_read_session_file` was local-cache-first. After teleport cleared the VPS cache, repopulation relied on the Stop hook POST. That POST could silently miss (crash, timeout, `/exit`+resume before the first assistant turn). Once anything read first, the SSH fallback cached whatever was on Mac Mini — which could itself be stale. The cache never re-validated.

**Fix:**
- New authoritative source: `_scan_latest_session_id(cwd, host=)` — finds the most recently modified JSONL under `~/.claude/projects/<slug>/` on the worker's current host (local path for local workers, single `ls -1t | head -1` SSH for teleported).
- `get_claude_session_id(name, authoritative=False)` — new flag. Non-authoritative (default) keeps fast-path local-cache reads for display UI. Authoritative always scans and refreshes the cache. It falls back to stale cache only if the scan fails.
- Correctness-critical call sites now use `authoritative=True`: `/rewind` resolution, `/restart` (direct + remote), transcript HTML endpoint, `start_worker` resume path, `_stop_worker_for_teleport`.
- The cache is now an optimization, not a source of truth. Stale cache self-heals on the next authoritative read.
...
- Per-worker in-flight restart lock: `_restart_in_progress` dict with threading.Lock prevents concurrent/queued restarts from firing at the same time.
...
- `--force` bypasses the "already running" guard but NOT the in-flight dedupe (separate concerns).
...
- Chunked endpoint splits text into sentences, synthesizes each, then concatenates with ffmpeg.
- TTS timeout increased to 60s (runs in background thread, does not block response).
...
- Reply-forwarded voice messages also get transcribed.

**TTS (outgoing text → voice):**
- Workers add `[[speak]]` at end of response to send voice alongside text.
- `[[speak:custom summary]]` speaks different text than what is displayed.
- Text is always sent first. Voice is synthesized in a background thread via Qwen3-TTS API (Mac Mini :10111).
- Fail-open. If TTS is unavailable, text still sends normally.
...
- `STT_TIMEOUT` / `TTS_TIMEOUT` — fail-open timeouts (10s / 60s)

**Architecture (Codex-reviewed):**
- Thin provider functions (`transcribe_voice`, `synthesize_speech`). No class hierarchy.
- STT runs inline (preserves message ordering). TTS runs async (background thread).
- Provider endpoints come from env vars. No auto-fallback to cloud (privacy/cost).
...
**Bug fix:** `export_hook_env()` exported `BRIDGE_URL=http://localhost:8271` to remote workers. Localhost does not resolve to VPS from Mac Mini. Now uses `BRIDGE_PUBLIC_URL` (Tailscale IP) for remote hosts. Keeps localhost for local workers. Also fixed pre-existing test failure in `test_remote_dispatch_export_hook_env` (mock needed echo $HOME handling). FAST 236/1.

### v0.28.1 - Fix 11 teleport gaps (Codex-audited)

**Systematic fix of all remaining teleport-unaware code paths.** Codex audit identified the gaps.
...
- `export_hook_env()`: remaps SESSIONS_DIR to remote `$HOME` prefix (for example, `/Users/beastoinagents/...`).
...
**New feature: Git-based teleport sync** replaces rsync for git repositories. VPS hosts bare repos at `~/git-server/<project>.git`. Workers push WIP state (via `git stash create`) to per-worker branches. The target fetches deltas. ~0-50s vs 600s+ for rsync over Tailscale on large repos.
...
**Key design:** `git stash create` does not support `-u` for untracked files. Workaround: temporarily `git add -A`, create stash, then `git reset HEAD` + re-stage original files. The source tree is never mutated.

**Bug fix: Checkin CWD validation for teleported workers.** `/checkin?cwd=` rejected remote paths (for example, `/Users/beastoinagents/omi/omi-ren`) because `validate_cwd()` checked the VPS filesystem. Now uses `get_worker_host()` to detect teleported workers. Validates via SSH on the remote host.
...
**Bug fixed:** Long text messages sent via bridge get pasted into Claude Code's input as `[Pasted text #N +M lines]` but the Enter key is swallowed. The text sits in input without submitting. Root cause: `paste-buffer -r` sends raw text without bracketed paste control codes. Claude Code's time-based paste detection treats the immediately-following Enter as part of the paste.

**Fix:**
- Add `-p` flag to `paste-buffer`. This sends proper bracketed paste codes (`\e[200~`...`\e[201~`) so TUI apps know where the paste ends.
...
- **Backend source drift**: `get_worker_backend()` checked session dict before backend file. This allowed drift when registry/RAM state got stale. Fix: backend file (`SESSIONS_DIR/<name>/backend`) is now the canonical source. Session dict is fallback only.
...
**Bug fixed:** When multiple processes send to the same tmux session concurrently (worker-to-worker or bridge+worker), the `load-buffer → paste-buffer → send-keys Enter` sequence interleaves. Text from different senders concatenates on one line. Orphan Enters submit empty input. 93% corruption rate in chaos testing.
...
- **`/workers` `send_example`** now wraps tmux commands in `flock`. Inter-worker sends use the same lock file.
- Lock files are node-namespaced (multi-node isolation, same pattern as pipe isolation).
...
- `should_run_test()` / `run_test()` helpers in test.sh. All test invocations go through `run_test()`.
...
**New features:**
- **Checkin CWD**: Workers can shift their working directory via `curl checkin?name=X&cwd=/path`. The bridge validates the path, stores it in RAM, and restarts the worker in the new directory if it differs from the current tmux pane CWD.
...
**New features:**
- **Persistent worker registry (`workers.json`)**: The bridge remembers all hired workers in `NODE_DIR/workers.json`. When a worker's tmux session dies (context limit, crash, idle timeout), the bridge still knows about it and can recover it.
...
- **EXITED watchdog state**: New watchdog state for workers in the registry but with no tmux session. Triggers an alert with a suggested `/restart <name>` action.
- **`/team` shows exited workers**: Workers whose tmux sessions have died appear as "exited" in `/team` output.
...
- **First-run bootstrap**: If `workers.json` does not exist, the bridge auto-creates it from current tmux sessions. Existing deployments do not lose track of workers.
...
**New features:**
- **SessionStart hook (`checkin-on-start.sh`)**: Auto-runs `/checkin` after `/compact` or `/resume`. Re-injects bridge instructions (file sending, `/workers` discovery, name prefix rule) into Claude's context. Workers no longer lose instructions after compaction.
- **Matcher-based firing**: Hook uses `compact|resume|init|start` matcher. Fires on compaction, resume, init, and start events.
- **`hook install` installs both hooks**: Stop hook (response forwarding) and SessionStart hook (auto-checkin) are installed together.
- **Checkin notes (file-based)**: The bridge reads `TEAM_DIR/checkin-note.txt` (`TEAM_DIR` env var, default `~/team`) and appends it to all `/checkin` responses and `/hire` welcome messages. Supports `{name}` placeholder for per-worker substitution. The file IS the API — edit it directly. No HTTP endpoints needed.
...
- Auto-install on `run` checks for both hook files.
...
**New features:**
- **MCP tool inventory injection**: Claude workers inject a sanitized MCP tool inventory into the system prompt on every start/resume. Resumed sessions immediately know available tools.
...
**Architecture changes:**
- **MCP config parser**: Reads MCP servers from Claude settings and project `.mcp.json` (jsonc supported). Labels sources and redacts secrets.
- **Unified Claude start command**: Centralized command builder for both direct and Docker starts. Docker resume now propagates the session id and appended system prompt.
...
**New features:**
- **Worker health watchdog thread** (Phase 1): Background daemon samples tmux pane PIDs, Claude PIDs, child counts, and CPU stats every 4s. Computes worker health states.
...
- **Proactive Telegram alerts** (Phase 2): Watchdog sends alerts to admin when workers enter DEAD/STUCK/POISONED states. 3-min cooldown per worker. Sends "resolved" alerts when workers recover.
- **POISONED state detection** (Phase 3): When a worker is STUCK, the watchdog inspects tmux pane output (interactive) or `adapter.log` (non-interactive) for repeated error signatures (API errors, overloaded, rate limits). 3+ matches of any pattern = POISONED.
...
- **`_handle_watchdog_transition()`**: Extracted transition logic for alert/resolved decisions. Enables unit testing.
...
**New features:**
- **Adapter stderr logging**: Non-interactive backend adapters (codex, gemini, opencode) now log stderr to `<sessions_dir>/<worker>/adapter.log` (append mode). Previously stderr went to `/dev/null`. Adapter failures were invisible.
- **`_spawn_adapter()` helper**: Consolidated duplicate Popen logic from three backend classes into one helper. Handles adapter discovery, log file opening, process spawning, and PID tracking.
...
**New features:**
- **Adapter PID tracking**: Non-interactive backend `send()` methods store the subprocess PID in `_adapter_pids` dict.
- **`kill_adapter(name)`**: Terminates an inflight adapter process (terminate → wait → kill fallback).
- **`/pause` kills adapter**: For non-interactive backends, `/pause` kills the running adapter before clearing pending. Prevents stale responses.
...
- **Backpressure for non-interactive**: Rejects new messages while a non-interactive worker is still processing. Prevents silent adapter queuing.
...
- **400-fallback**: If Telegram rejects HTML (parse error), the bridge retries as plain text automatically. No more lost messages.
...
**Changes:**
- **Photos: no path restrictions.** Removed the allowed-directory check (was limited to /tmp, sessions dir, cwd). Any readable file can now be sent as a photo.
- **Documents: blocklist-only.** Removed the extension allowlist. Documents now only check against the blocklist of sensitive extensions (.pem, .key, .env, etc.) instead of requiring a known extension.
...
**New features:**
- **`GET /checkin` endpoint**: Workers can `curl -s $BRIDGE_URL/checkin?name=lee` to refresh bridge instructions at any time. Returns plain text with labeled sections (RECEIVING FILES, SENDING FILES, MESSAGING WORKERS, NAME PREFIX, REFRESH INSTRUCTIONS).
- **Welcome on restart**: `restart()` re-sends the welcome message after resume or relaunch. Workers always get fresh instructions.

**Improvements:**
- **Structured welcome message**: Instructions now have labeled sections instead of a wall of text.
- **`_build_welcome()` method**: Welcome message extracted into a shared method. Used by `hire()`, `restart()`, and `/checkin`. Single source of truth for instructions.
...
- **Session ID persistence**: Stop hook (`send-to-telegram.sh`) captures session UUID from transcript path. Saves to `claude_session_id` file per worker.
- **CWD persistence**: Working directory saved at hire time and in stop hook fallback. Enables cross-directory resume (`--resume` requires matching cwd).
- **`/progress` shows resume status**: Displays "Resume: available (session abc12345...)" or "Resume: not available".

**Improvements:**
- **Simplified worker messaging instructions**: Welcome message directs workers to call `GET /workers` for ready-to-use send commands. Workers no longer need to memorize tmux vs pipe syntax.
...
**New features:**
- **Backend Protocol** (`typing.Protocol`) with `name`, `is_interactive`, `start_cmd(resume_id="")`, `send()`, `is_online()`.
...
- **WorkerManager**, **TelegramAPI**, **CommandRouter** classes for clearer separation.
...
- Interactive/non-interactive detection is backend-driven (`backend.is_interactive`). Replaces hardcoded backend checks.
...
- Codex lifecycle cleans metadata on `/end` and resets session ID on `/restart --clean`.
...
**New features:**
- `/hire` supports worker backend selection:
  - `--codex` flag (for example, `/hire --codex alice`)
  - `codex-<name>` prefix (for example, `/hire codex-alice`)
- `/team` and `/progress` show worker backend metadata.

**Architecture changes:**
- Backend metadata stored per tmux session via `WORKER_BACKEND`. Read during session scans.
...
**New features:**
- **Last known chat ID persistence**: Saves chat ID to file. On restart, the bridge sends "I'm online" to the last known chat.
- **Last active worker persistence**: Saves focused worker name to file. On restart, the bridge restores focus to that worker.
...
- Chat ID updates on every admin message (stays fresh).
- Active worker saves on `/hire`, `/focus`, and worker slash commands.

**Why:**
- Before this change, after a bridge restart you had to send a message to the bot to know it was online.
- Before this change, after a bridge restart the focus was lost. You had to `/focus <name>` again.
- Now the bridge notifies you proactively and restores your workflow.

### v0.16.4 - Orphan detection, status improvements, tmux delay fix

**Fixes:**
- Added 0.2s delay between `tmux send-keys` text and Enter. Prevents race condition where text and Enter interleave.

**New features:**
- `status --all` detects orphan processes (tunnels/bridges not owned by any node).
...
- Exit cleanly on webhook setup failure (do not leave orphan processes).
...
- Empty prompt = message submitted → show 👀.
- Text still in prompt = submit failed → no reaction.

**Why:** Previously, reaction appeared immediately after `tmux send-keys` succeeded. This did not guarantee Claude Code processed the Enter as a submit (could be in multi-line mode, busy, etc.).
...
**New feature:** Workers can now connect to remote bridges. Enables distributed deployments.

| Config | Use Case |
|--------|----------|
| `PORT=8270` (default; prod node uses 8271) | Local setup, hook builds `http://localhost:{PORT}/response` |
...
**Backward compatible:** Existing setups that use only `PORT` continue to work unchanged.

### v0.13.2 - Fix hook template causing duplicate messages

**Bug fix:** Per-node hooks used template mode defaults instead of baked config.

The issue was overly complex conditional logic that broke after sed substitution. All three hooks (prod/dev/sandbox) matched the same sessions → duplicates.

**Fix:** Simplified hook template. Removed all conditionals. Use baked values directly:
```bash
...
**Bug fix:** `hook install` generated incorrect Claude Code settings.json structure.
...
- New installs create proper `hooks.Stop[0].hooks[]` array.
...
**CLI flags:**
```bash
--sandbox              # Enable (disabled by default)
--no-sandbox           # Disable
...
**New feature:** Workers can send documents back to Telegram with `[[file:/path|caption]]` tag.
...
**New feature:** Manager can send any file type to workers, not just images.

- **PDF, documents, code files** — all file types accepted and downloaded to worker's inbox.
...
- No more `--dangerously-skip-permissions` needed when sandbox is used.
- `--sandbox` / `--no-sandbox` flags to toggle.
- `--sandbox-image` to set Docker image.
...
**New feature:** Long responses are automatically split into multiple messages to fit Telegram's 4096 character limit.
...
Previously, `--port=1789` was silently ignored (falling back to node config). This caused confusing errors like "Port 8271 is already in use" when you specified a different port.
...
**Why:** Less magic. More predictable. If port is busy, the user decides what to do.
...
- Bridge logs timestamp, parent PID, and parent cmdline on SIGTERM.
- Helps identify what process or script triggered unexpected shutdowns.

**Logging improvements:**
- Both `start` and `run` commands write bridge output to log file.
...
- Detects if port is held by the bridge vs another process.
- Auto-restarts old bridge gracefully (no user action needed).
- Falls back to suggesting alternative port if not the bridge process.
...
**Test improvements:**
- E2E image test skips gracefully when no real `TEST_CHAT_ID` is provided.
...
- Bot command list updates when workers are hired or offboarded.
...
- **Removed playbook persistence**: `/learn` is prompt-only. Does not write to disk.
...
- **Fix**: Hook polls transcript until stable (file size unchanged + content parseable).
...
- Team playbook management is external (for example, `~/team/playbook.md`). Not bridge's responsibility.

### v0.9.2 - Fix tmux send race condition

**Problem:** Concurrent messages to the same tmux session could interleave. Messages corrupted each other. About 50% of messages failed under rapid load.

**Root cause:** The two-call pattern (`tmux_send` + `tmux_send_enter`) was not atomic. When multiple threads sent messages to the same session at the same time, calls interleaved (for example, text1, text2, Enter1, Enter2).
...
**New command:**
- `/learn` — Ask focused worker about today's learnings (prompt-only, no persistent storage). Note: removed in v0.23.0.
...
- Team playbook managed externally (for example, `~/team/playbook.md`). The bridge does not persist.
...
**Changes:**
- Pending files auto-delete after 10 minutes.
- Reverted double Enter (did not help with batching).
- Fixes "busy" status getting stuck when hooks do not fire.

### v0.6.4 - Fix message batching with double Enter

**Bug fix:**
- Bridge sends double Enter when routing messages.
...
**Bug fix:**
- Bridge writes `port` file to node directory on startup.
...
- Hook sends to Telegram if `chat_id` exists, regardless of `pending` file.
- `pending` file is now only used for busy status indicator (UI). Not as a send gate.

**Why this change:**
- Fixes race condition where multiple rapid messages caused lost responses.
...
- Sessions without `chat_id` (non-Telegram) do not send to Telegram.

### v0.6.1 - Fix pending file cleanup

**Bug fix:**
- Stop hook cleans up `pending` file after successfully sending response to bridge.
- Previously, sessions appeared "busy" forever because the pending file was never removed.
...
- **Smart auto-detection**: If only one node runs, uses it. If multiple, prompts or errors.
...
**Why `--no-sandbox` for prod/dev/test?** Docker overhead impacts performance. Use the sandbox node for isolation when running untrusted code.
...
**Improvements:**
- **Tunnel watchdog checks reachability**: Previously only checked if cloudflared process was alive. Now also curls the tunnel URL to verify it is actually reachable.
- **Kills zombie tunnels**: If the tunnel process is alive but the URL is unreachable, kills the process and restarts.
...
**Hook improvements:**
- Hook reads `TMUX_PREFIX` env var for session detection.
...
- **Admin auto-learn**: First user to message becomes admin. The bridge persists the admin ID in `last_chat_id` and restores it on restart when `ADMIN_CHAT_ID` is not set.
- **Silent rejection**: Non-admin users get no response (bot does not reveal itself).
...
Claude (has token)                   Claude (NO token)
    │                                    │
    └─► Hook calls Telegram API          └─► Hook POSTs to localhost:{PORT}/response
                                              │
                                              ▼

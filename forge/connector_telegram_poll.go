...
// TelegramPollConnector connects directly to the Telegram Bot API using
// getUpdates long-polling. For workers without a public IP.
//
// Inbound: parses text, photos, documents, voice, audio, video, stickers.
// Downloads files via getFile API. Voice messages are delivered as file paths.
//
// Outbound: sends text with HTML parse_mode (Markdown fallback to plain text).
// Splits long messages (4096 char limit). Parses [[image:]] and [[file:]] tags
// and sends via sendPhoto/sendDocument/sendVideo/sendAudio/sendVoice.
//
...
func (t *TelegramPollConnector) Send(ctx context.Context, resp Response) error {
	...
	// Parse and extract [[image:path|caption]] tags
	var images []mediaTag
	...
	// Parse and extract [[file:path|caption]] tags
	var files []mediaTag
	...
	// Send files (routed by extension)
	for _, f := range files {
...
// sendText sends a text message. Tries HTML parse_mode first, falls back to plain.
...
// sendPhoto sends a photo via multipart upload.
...
// parseInboundMessage converts a Telegram message into an InboundMessage.
// Handles text, photos, documents, voice, audio, video, stickers.
...
// downloadFile downloads a Telegram file to the local inbox.
func (t *TelegramPollConnector) downloadFile(fileID string) string {
	...
	url := fmt.Sprintf("https://api.telegram.org/bot%s/getFile", t.token)
	...
	dlURL := fmt.Sprintf("https://api.telegram.org/file/bot%s/%s", t.token, result.Result.FilePath)
...
// splitTelegramMessage splits text into chunks respecting the max length.
// Prefers splitting at blank lines, then newlines, then spaces.
